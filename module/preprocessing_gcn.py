...
class RGCNModel(torch.nn.Module):
    def __init__(self, in_channels, out_channels, num_relations):
        ...
        self.rgcn = RGCNConv(in_channels, out_channels, num_relations=num_relations)

    def forward(self, x, edge_index, edge_type):
        return self.rgcn(x, edge_index, edge_type)

def create_speaker_relation_graph(data_file, device, max_seq_len):
    f = open(data_file)
    data = json.load(f)
    f.close()
    ...
    for conversation in data.values():
        for turn_list in conversation:  
            for turn_data in turn_list:  
                ...
                encoded = tokenizer.encode_plus(utterance, add_special_tokens=True, max_length=max_seq_len,
                                                pad_to_max_length=True, return_attention_mask=True, return_tensors='pt')
                embedding = encoded['input_ids'].squeeze(0)
                speaker_id = 0 if speaker == 'A' else 1
                tokenized_data.append((embedding, speaker_id))
    ...
    for i, (embedding, speaker_id) in enumerate(tokenized_data):
        x.append(embedding)
        if speaker_id not in speaker_to_index:
            speaker_to_index[speaker_id] = []
        speaker_to_index[speaker_id].append(i)

    for speaker_id, indices in speaker_to_index.items():
        for i in range(len(indices) - 1):
            edge_index.append((indices[i], indices[i + 1]))
            edge_index.append((indices[i + 1], indices[i]))
            edge_type.extend([speaker_id, speaker_id])  

    x = torch.stack(x)
    edge_index = torch.tensor(edge_index, dtype=torch.long).t()
    edge_type = torch.tensor(edge_type, dtype=torch.long)

    graph_data = Data(x=x, edge_index=edge_index.to(device), edge_attr=edge_type.to(device))
    return graph_data
...
def get_data(data_file, device, max_seq_len, contain_context=False):
    ...
    rgcn_graph_data = create_speaker_relation_graph(data_file, device, max_seq_len=max_seq_len)
    rgcn_model = RGCNModel(in_channels=768, out_channels=128, num_relations=2)  
    rgcn_model = rgcn_model.to(device)
    rgcn_output = rgcn_model(rgcn_graph_data.x, rgcn_graph_data.edge_index, rgcn_graph_data.edge_attr)
    ...
    return preprocessed_utterance, rgcn_output.to(device), out_speaker.to(device), out_emotion_label.to(device), out_pair_cause_label.to(device), out_pair_binary_cause_label.to(device)
